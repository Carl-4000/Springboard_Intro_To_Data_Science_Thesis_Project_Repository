---
title: "Basketball End-Game Analysis"
author: "Carl Larson"
date: "5/22/2018"
...
Given the Minutes Remaining, and the score, it's possible to use this classifier to help head coaches optimize declaring Garbage Time as early as possible to *minimize injury risk*, while still *maximizing* likelihood of winning the game. Between these two tensions is the heart of this project. 
...
![Heaviliy skewed-right](https://github.com/Carl-4000/Springboard_Intro_To_Data_Science_Thesis_Project_Repository/blob/master/nba_salary_hist.png?raw=true)

Below we see a glimpse of NBA salaries broken down by age as a scatter plot. [source](https://github.com/Flood1993/nba_salaries_analysis). 
...
Basketball being a timed sport, the teams can only score at a certain rate, which this project inspects in detail in the Regression Addendum. Thus there is an envelope where the game goes from being classified as competitive, to out-of-reach for the trailing team simply due to the speed of time, and the speed of basketball. 
...
The first graph used to explore this data (via simple functions like GFMapper and ContMapSlicer) happened to be a style of presentation already published by Franklin Kenter in a paper [ESPN would later call "hacking basketball."](http://www.espn.com/nba/story/_/id/12954130/let-hacking-begin) 

![Kenter's Choice of Axes on Figure 1 of his paper frame the graph as points difference and time](https://github.com/Carl-4000/Springboard_Intro_To_Data_Science_Thesis_Project_Repository/blob/master/Kenter_graph.png?raw=true)

![Goel's Contour Plot](https://github.com/Carl-4000/Springboard_Intro_To_Data_Science_Thesis_Project_Repository/blob/master/Goel_Contour_Plot.png?raw=true)

![Goel's Formula vs Michael Lewis's Reported Rule of Thumb](https://github.com/Carl-4000/Springboard_Intro_To_Data_Science_Thesis_Project_Repository/blob/master/Goel_Formula_Plot.png?raw=true)
...
GFmapper <- function(k = 35){
  ...
  for(i in 1:k){
    h <- h
    dfJ <- as.matrix(algo2app(h=h, p = scdiff, m = 90))  
    matx[i,1] <- dfJ[1,1]
    matx[i,2] <- dfJ[1,2]
    matx[i,3] <- dfJ[1,3]
    matx[i,4] <- dfJ[1,4]
...
```{r mapSlicer}
# Below is a simple function that merely graphs the function above
...
ContMapSlicer()  #including a sample call of this function
```
...
One early source of data validation comes from the sports statistics blog 'inpredictable,' where Michael Beuoy (also of the blog 538) hosts a live win probability calculator web application for NBA games, as well as a similar calculator that takes user inputs for the variables of current possession, as well as time and score difference, and outputs one probability that the leading team will win [source](http://www.inpredictable.com/2015/04/live-win-probabilities-for-nba-playoffs.html). In closer inspections with the results of Beuoy's application and Hooply for different scenarios, it does seem that Hooply is a few percentage points biased towards the leading team, but not by more than a few percent. It's validating to see that Hooply is highly correlated with Beuoy's app on win probability, and serves as a nice milepost for users who will be interested to compare a second opinion. This also supports the Goel analysis comparison that suggested Hooply was biased for the leading team by 14% at 7 minutes left with a 12 point lead. 

Fortunately in the way Hooply is coded, users are incentivized to better-understand Hooply's bias for one team or another, and thus it's valuable to know Hooply's relative bias to similar products, because users can simply adjust the "confidence interval" input of Hooply to reflect their interpretation of Hooply's bias. As the author, my expectation that Hooply was somewhat biased toward the leading team was reflected by my setting of the default value for confidence interval to well above 99%, so that before a game is declared as garbage time, there must be 
...
Rather than taking an index as a time metric like in algo2app, algo3app here actually takes the users' actual reported time remaining and automatically converts it, using the linear regression logic from the regression addendum. 
...
gftopo4 <- function(m=60){
  ...
  contour(x = xax, 
          ...
          z = dfz, levels = seq(from = 0.9, to = 1, by = 0.025), 
          col = topo.colors(5),
...
# those 2 matrices (1 of which is a vector) to build the contour plot
...
server <- function(input, output) {
  ...
  output$contourP <- renderPlot({
    ...
    yax <<- as.vector(seq(3,20))  #score difference vector to create the y-axis
    ...
    contour(x = (rev(xax[,-1]))*-1, 
            y = yax, 
            z = (apply(dfz[,-1], 2, rev)),
            levels = c(seq(from = 0.9, to = 0.975, by = 0.025), 1),
...
Data mining for this topic was an ongoing obstacle for this project, and I personally see the need to expand my programming skill-set to encompass advanced data scraping and mining so I can access the data at [PopcornMachine.net](http://popcornmachine.net).
